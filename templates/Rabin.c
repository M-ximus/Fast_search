...
int temple_func(const char* file_name, hash_table* info_table);
...
int main(int argc, char* argv[])
{
    ...
    int ret = collect_dir_info("templates/", info_table);
    assert(ret == 0);


    ret = file_func("Rabin.c", info_table);
    assert(ret == 0);
...
int collect_dir_info(const char* dir_name, hash_table* info_table)
{
    ...
    errno = 0;
    int ret = 0;
    while((temple_info = readdir(dir_fd)) && ret == 0)
    {
        if (temple_info->d_type == DT_REG)
        {
            ret = temple_func(temple_info->d_name, info_table);
            printf("%s\n", temple_info->d_name);
        }
    }
    if (errno != 0)
...
{
    if (file_name == NULL || file_size == NULL || fd == NULL)
        return NULL;
